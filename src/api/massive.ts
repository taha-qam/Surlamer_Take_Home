...
export async function searchTickers(query: string, limit = 20): Promise<TickerInfo[]> {
  const data = await get<{ results?: TickerInfo[] }>('/v3/reference/tickers', {
    search: query, market: 'stocks', active: 'true', limit: String(limit), sort: 'ticker'
  });
  return data.results ?? [];
}
...
export async function getSnapshots(tickers: string[]): Promise<TickerSnapshot[]> {
  ...
  const data = await get<{ tickers?: TickerSnapshot[] }>('/v2/snapshot/locale/us/markets/stocks/tickers', {
...
export async function getSingleSnapshot(ticker: string): Promise<TickerSnapshot | null> {
  try {
    const data = await get<{ ticker?: TickerSnapshot }>(`/v2/snapshot/locale/us/markets/stocks/tickers/${ticker}`);
    return data.ticker ?? null;
  } catch { return null; }
}

export async function getAggregates(ticker: string, from: string, to: string, timespan = 'day'): Promise<AggBar[]> {
...
export async function getGainersLosers(direction: 'gainers' | 'losers'): Promise<TickerSnapshot[]> {
  const data = await get<{ tickers?: TickerSnapshot[] }>(`/v2/snapshot/locale/us/markets/stocks/${direction}`);
  return data.tickers ?? [];
}

export async function getMarketStatus(): Promise<{ market: string; serverTime: string; exchanges: Record<string, string> }> {
...
export async function getMarketNews(limit = 6): Promise<NewsItem[]> {
  const data = await get<{ results?: NewsItem[] }>('/v2/reference/news', {
    limit: String(limit), sort: 'published_utc', order: 'desc'
  });
  return data.results ?? [];
}
